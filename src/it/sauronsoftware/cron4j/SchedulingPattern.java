...
public class SchedulingPattern {
    ...
    public Date getNextValidDateAfterStartDate(Date startDate){

        Calendar c=Calendar.getInstance();
        
        c.setTime(startDate);
        
        int steppingUnit=getSteppingUnit(c);        

        while(true){
            if(this.match(c.getTimeInMillis()))
            {
                if(c.getTime().after(startDate))
                {  break;  }
                c.add(steppingUnit, 1);
            }else{
                c.add(steppingUnit, 1);
            }
        }
    ...
    public Date getNextValidDate(){
        ...
        return getNextValidDateAfterStartDate(new Date());
    }

    private int getSteppingUnit(Calendar c) {

        String minuteType="";
        if(minuteMatchers.get(0) instanceof AlwaysTrueValueMatcher){
            minuteType="AlwaysTrueMatcher";
        }else if (minuteMatchers.size()==1){
            minuteType="SingleValueMatcher";
        }

        String hourType="";
        if(minuteMatchers.get(0) instanceof AlwaysTrueValueMatcher){
            hourType="AlwaysTrueMatcher";
        }else if (minuteMatchers.size()==1){
            hourType="SingleValueMatcher";
        }

        if(minuteType.equals("SingleValueMatcher")&&hourType.equals("SingleValueMatcher"))
        {
            c.set(Calendar.HOUR_OF_DAY,((IntArrayValueMatcher)hourMatchers.get(0)).get(0));
            c.set(Calendar.MINUTE,((IntArrayValueMatcher)hourMatchers.get(0)).get(0));

            return Calendar.DATE;
        }

        if(minuteType.equals("SingleValueMatcher")&&hourType.equals("AlwaysTrueValueMatcher"))
        {
            c.set(Calendar.MINUTE,((IntArrayValueMatcher)minuteMatchers.get(0)).get(0));

            return Calendar.HOUR_OF_DAY;
        }

        if(minuteType.equals("AlwaysTrueValueMatcher")&&hourType.equals("SingleValueMatcher"))
        {
            c.set(Calendar.HOUR_OF_DAY,((IntArrayValueMatcher)hourMatchers.get(0)).get(0));
        ...
        if(minuteType.equals("AlwaysTrueValueMatcher")&&hourType.equals("AlwaysTrueValueMatcher"))
        {
            return Calendar.DATE;
        }
